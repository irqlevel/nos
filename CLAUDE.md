...
- `src/cpp/net/` — device abstraction, TCP, HTTP client (`http.cpp`, with a transport seam so the same client speaks plain TCP or TLS), the TLS session wrapper over the Rust client (`tls.cpp`), UDP shell, netconsole (kernel log over UDP)
