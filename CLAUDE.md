...
- `src/cpp/net/` — the device abstraction and the frame pool, the HTTP client (`http.cpp`, with a transport seam so the same client speaks plain TCP or TLS) and the TLS session wrapper over the Rust client (`tls.cpp`); **everything from the wire formats up to and including the UDP protocols is Rust** (`src/rust/net`), reached through one file each here
