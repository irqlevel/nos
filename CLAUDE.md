...
- `src/cpp/drivers/` — serial, console (`screen.cpp` picks EGA text on BIOS vs. the 8x16-font pixel framebuffer under UEFI), PIT/HPET/RTC, 8042 keyboard, `usb/` (xHCI host controller + HID boot keyboard, for UEFI laptops with no PS/2), PCI, MSI-X, ACPI, virtio (net/scsi) behind the `VirtioTransport` interface (legacy+modern virtio-pci on x86, virtio-mmio on arm64) — **the foundation also exists in Rust** (`src/rust/virtio`: the virtqueue, both buses), where **virtio-blk** and **virtio-rng** already live (`src/rust/drivers/`)
