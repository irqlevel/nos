...
- `src/cpp/fs/` — the filesystems and the C++ way into the VFS (**the VFS and ext2 are Rust**, `src/rust/fs`: the mount table, path resolution, open handles and the file API, over an ops table each filesystem gives it, and the filesystems below it: **ext2 read-write** -- the root, which `root=auto` mounts by the label `nos`; `fs/rootfs.cpp` picks it -- **ramfs** and **procfs**) — mount points and a file API (`Open/Read/Write/Seek`, `Stat`, `ReadDir`, `Rename`, `Truncate`), nanofs (on-disk, small), the `fstest` self-test (`fs/fstest.cpp`); see `docs/filesystems.md`
