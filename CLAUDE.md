...
- **`ffi/`** holds `extern "C"` declarations and `#[repr(C)]` structs and no logic. Every pointer is raw (`*mut u8`, `*const u8`): no reference crosses the boundary. A kernel object is an opaque `usize` handle, 0 for null or failure.
