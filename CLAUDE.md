...
- `src/cpp/block/` — one header, `block.h`: the `kernel_blockdev_*` ABI the C++ that is left (the shell, the disk log, a mount) calls. There is no C++ view of a device — a device is the handle the table knows it by; **the block layer is Rust** (`src/rust/block`): the device table, its claims, the partition tables, and the async request queue the drivers use
