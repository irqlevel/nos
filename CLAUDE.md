...
- `src/cpp/fs/` — the filesystems and the C++ way into the VFS (**the VFS itself is Rust**, `src/rust/fs`: the mount table, path resolution, open handles and the file API, over an ops table each filesystem gives it) — mount points and a file API (`Open/Read/Write/Seek`, `Stat`, `ReadDir`, `Rename`, `Truncate`), ramfs, nanofs (on-disk, small), **ext2 read-write** (the root filesystem: `root=auto` mounts the ext2 labelled `nos`; `fs/rootfs.cpp`), procfs, the `fstest` self-test (`fs/fstest.cpp`); see `docs/filesystems.md`
