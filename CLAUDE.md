...
- `src/cpp/fs/` — the filesystems and the C++ way into the VFS (**the VFS and ext2 are Rust**, `src/rust/fs`: the mount table, path resolution, open handles and the file API, over an ops table each filesystem gives it, and **ext2 read-write** below it -- the root filesystem, which `root=auto` mounts by the label `nos`; `fs/rootfs.cpp` picks it) — mount points and a file API (`Open/Read/Write/Seek`, `Stat`, `ReadDir`, `Rename`, `Truncate`), ramfs, nanofs (on-disk, small), procfs, the `fstest` self-test (`fs/fstest.cpp`); see `docs/filesystems.md`
