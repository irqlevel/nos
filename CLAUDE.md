...
- `src/cpp/block/` — one header, `block.h`: the `kernel_blockdev_*` ABI, for the one boot-time call C++ still makes. There is no C++ view of a device — a device is the handle the table knows it by; **the block layer is Rust** (`src/rust/block`): the device table, its claims, the partition tables, the async request queue the drivers use, the `disks`/`partitions`/`diskread`/`diskwrite` commands, and the disk log (`disklog.rs`)
