...
- **Style**: namespaces `Kernel::`, `Kernel::Mm::`, `Stdlib::`. Trace with `Trace(level, "fmt", …)` / Rust `trace!(...)`; level 0 is always visible and per-subsystem level constants (`KbdLL`, `UsbLL`, `PageAllocatorLL`, …) live at the top of `kernel/trace.h` — raise one there to debug a subsystem, or raise the global level on a running kernel with the `loglevel N` shell command (no rebuild, which is what you want on a box whose only console is a UDP socket). No magic numbers — name constants. Preserve existing formatting; don't reflow unchanged code. 4-space indent.
