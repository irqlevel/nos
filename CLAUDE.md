...
The traits' own comments (`block/src/table.rs`, `kcore/src/net.rs`, `kcore/src/entropy.rs`) are the contract; the smallest real one of each kind is the example: `drivers/virtio_blk` (block; `drivers/nvme` for the asynchronous path), `drivers/r8168` (a NIC), `drivers/virtio_rng` (an entropy source).
