...
| `dma` | `DmaBuffer` (contiguous; `bytes`/`bytes_mut`, and `load`/`store` of a `Pod` at an offset — volatile, aligned and bounds-checked), `PhysMapping`, `virt_to_phys`; for a descriptor ring, `Volatile<T>` cells, `unsafe trait Descriptor` and `DmaBuffer::leak_ring::<D>() -> (&'static [D], phys)` |
| `barrier` | `dma_wmb`, `dma_rmb` (`dmb oshst`/`oshld` on arm64) |
| `pod` | `unsafe trait Pod`, then `pod::read`/`write`/`zeroed`: how an on-disk or on-wire structure is read out of a buffer |
