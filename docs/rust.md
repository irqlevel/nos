...
| `vnic` | **for a module.** A virtual NIC in the kernel's stack, made the first time it is opened by name (`Vnic::open`): what the stack sends out of it goes to a `VnicSink` the module attaches, what the module hands in (`receive`) arrives as received. The hypervisor's `hv0` |
| `fs` | **for a module.** `read(path, max)`, `write(path, data)` (replaces, then syncs), `create`, `create_dir` — its configuration files; `read_at(path, ..)` — a file too large to read whole, a guest's kernel; `File` — a file held open, a guest's disk image: `read_at`, `write_at` (within the file's size, never growing it) and `sync` (the file's filesystem), the handle looked up by the kernel on every call, the file closed on drop and unremovable while open. The image opens files through the `fs` crate itself: `fs::vfs::Open::new(fs::vfs_instance()?, path, flags)` |
