...
| `fs` | **for a module.** `read(path, max)`, `write(path, data)` (replaces, then syncs), `create`, `create_dir` — its configuration files; `read_at`, `write_at` (within the file's size, never growing it) and `sync` — a file that is a disk, a guest's image. The image opens files through the `fs` crate itself: `fs::vfs::Open::new(fs::vfs_instance()?, path, flags)` |
