...
src/rust/
  ...
  modules/    Loadable modules, each built into a .ko (hello; sshd, an SSH server; blkload, a block I/O load test; netblk, a disk served over UDP, zero-copy; modtest, which the boot test loads; slowexit, an exit that takes its time)
