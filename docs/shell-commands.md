...
| `hv forward [add <port> <vm> <guest-port> \| del <port>]` | A port of nos's relayed to a port of a `net` guest's -- how a guest's server is reached from outside. `hv forward` alone lists them, with the connections each has carried (x86-64, AMD-V). See [A network](hypervisor.md#a-network) |
