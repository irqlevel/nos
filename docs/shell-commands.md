...
| `hv boot <bzImage> [mem=MiB] [secs=N] [cpu=N] [initrd=path] [disk=path]... [input=...] [cmdline=...]` | A Linux guest for `secs` (60 by default): the `bzImage` and initrd read from files into its memory, run on a vCPU, then its console and how it ended printed, the console also going to the kernel log a line at a time (`hvguest| ...`) as it runs. `input=` is typed at its shell once it has one, `\n` for a newline (x86-64, AMD-V). See [A Linux guest](hypervisor.md#a-linux-guest) |
| `hv start <bzImage> [mem=MiB] [cpu=N] [initrd=path] [disk=path]... [input=...] [log] [restart] [net] [cmdline=...]` / `hv list` / `hv console <id> [bytes=N]` / `hv attach <id>` / `hv send <id> <text>` / `hv exec <id> [secs=N] <line>` / `hv wait <id> [secs=N] <text>` / `hv restart <id>` / `hv stop <id\|all>` | A Linux guest that runs until it is stopped, on a vCPU task of its own bound to `cpu` or to the CPU with the fewest guests, and what reaches it while it runs: `list` says which run, how the others ended and how often each was restarted; `console` prints the end of one's console, escape sequences taken out; `attach`, from an SSH session with a terminal (`ssh -t`), is the console live and typed at, until ^] detaches; `send` types at it; `exec` types a line and prints the answer, up to the prompt printed after the line went in (10 s by default); `wait` returns once its console shows `text`, or it stops; `restart` boots it again from its files, running or stopped, and `restart` on `start` does that each time the guest resets itself (a reboot), at most five times a minute; `stop` stops it and prints how it ended. `log` also sends its console to the kernel log (`hvvm<id>| ...`). `hv off` will not turn the extension off under a running guest, and `rmmod hv` stops them all first (x86-64, AMD-V). See [Guests that stay up](hypervisor.md#guests-that-stay-up) |
