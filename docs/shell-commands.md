...
| `hv start <bzImage> [mem=MiB] [cpu=N] [initrd=path] [input=...] [log] [cmdline=...]` / `hv list` / `hv console <id> [bytes=N]` / `hv send <id> <text>` / `hv exec <id> [secs=N] <line>` / `hv wait <id> [secs=N] <text>` / `hv stop <id\|all>` | A Linux guest that runs until it is stopped, on a vCPU task of its own bound to `cpu` or to the CPU with the fewest guests, and what reaches it while it runs: `list` says which run and how the others ended; `console` prints the end of one's console, escape sequences taken out; `send` types at it; `exec` types a line and prints the answer, up to the prompt printed after the line went in (10 s by default); `wait` returns once its console shows `text`; `stop` stops it and prints how it ended. `log` also sends its console to the kernel log (`hvvm<id>| ...`). `hv off` will not turn the extension off under a running guest, and `rmmod hv` stops them all first (x86-64, AMD-V). See [Guests that stay up](hypervisor.md#guests-that-stay-up) |
