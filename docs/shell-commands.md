...
| `igbdump` | The igb chip's own state: MAC command/status registers, ring head and tail, receive statistics; the interrupt layout (INTx, one MSI-X vector, or a queue vector and another) with each vector's count, the throttle and the receive rate it follows, and how often under load the poll looked at an empty ring again rather than arm the interrupt, with the time that cost; and the PHY's view of the link — what it advertised, what the partner offered, both 1000BASE-T registers. What made the I210's 10 Mb/s latch diagnosable |
...
| `ps` | Show tasks |
| `top [ms]` | Per-task CPU use over a sampling window, percent per CPU (a busy thread reads 100%, a 20-CPU box tops out at 2000%), plus the number of tasks moved between CPU queues since boot, and of reschedules deferred because the task they landed on had preemption off, in the window and since boot -- how many of those on an idle task |
...
| `arp` | Show ARP table |
| `nat` | Whether NAT is on, between which devices and from which address; the first mappings, each with its protocol, the guest's side, where it goes, the external port it has and how long it has left; and what NAT has done -- packets out and back, mappings made, and those it had no mapping, next hop or frame for, or does not take. The hypervisor's switch turns it on for its guests. See [The way out](hypervisor.md#the-way-out-nat-dhcp-and-dns) |
...
| `netload start [port] [sink]` / `netload send <ip> <port> [size=64] [pps=0] [secs=0] [count=0] [tasks=1] [sport=9998]` | From the `netload` module, once it is loaded: a UDP load test from either end, so `profile` has something to look at other than an idle machine. `start` is the target: it echoes every datagram from the receive softirq (or drops it, in sink mode; port 9999 unless told), and can be driven from a host too, with `scripts/netload.py` (`--pps` to hold a rate, `--threads` to raise the ceiling). `send` is the source: `tasks` senders build datagrams of `size` bytes of payload straight into frames off the pool and hand them to the NIC a batch at a time -- as fast as it takes them, or `pps` a second -- until `count` are sent, `secs` have passed or `netload stop`; what comes back to `sport` is counted as echoes and never answered. It leaves room in the transmit queue for everybody else, so the shell goes on answering under it. See [Loadable modules](modules.md#netload) |
| `netload` / `netload stop` / `netload reset` | What the target and the source have counted -- packets, bytes, failures, rates, and which CPUs did the work; stop both; clear the counters |
...
| `hv [info]` / `hv on [cpu\|all]` / `hv off [cpu\|all]` / `hv run <guest\|all> [cpu]` | From the `hv` module, once it is loaded: the CPU's virtualization extension, and guests under it. `hv` says which extension the machine has and which CPUs it is on for -- asked of the CPUs themselves, by an IPI that reads the register, so a disagreement with what the module turned on is reported rather than hidden. `hv info` adds everything the CPU said about it, a feature a line: nested paging, next-RIP save, ASIDs, x2APIC, 1 GiB pages, and on arm64 the exception level the kernel is at and what stage-2 would translate into. `hv on` and `hv off` turn it on and off for one CPU or every running one; `rmmod` turns it off for all of them. `hv run` runs a built-in guest (`exits`, `uart`, `hypercall`, `fault`, `absent`, `triple`, `refused`, `spin`, `asid`), or all of them, each in a VM of its own on a task bound to `cpu` when one is named, and says what it did and whether that was what it was told (x86-64, AMD-V). See [The hypervisor](hypervisor.md) |
| `hv boot <bzImage> [mem=MiB] [secs=N] [cpu=N] [initrd=path] [disk=path[:ro]]... [input=...] [cmdline=...]` | A Linux guest for `secs` (60 by default): the `bzImage` and initrd read from files into its memory, run on a vCPU, then its console and how it ended printed, the console also going to the kernel log a line at a time (`hvguest| ...`) as it runs. `input=` is typed at its shell once it has one, `\n` for a newline (x86-64, AMD-V). See [A Linux guest](hypervisor.md#a-linux-guest) |
| `hv start <bzImage> [mem=MiB] [cpu=N] [initrd=path] [disk=path[:ro]]... [input=...] [log] [restart] [net] [cmdline=...]` / `hv list` / `hv console <id> [bytes=N]` / `hv attach <id>` / `hv send <id> <text>` / `hv exec <id> [secs=N] <line>` / `hv wait <id> [secs=N] [boot=N] <text>` / `hv restart <id>` / `hv stop <id\|all>` | A Linux guest that runs until it is stopped, on a vCPU task of its own bound to `cpu` or to the CPU with the fewest guests, and what reaches it while it runs: `list` says which run, how the others ended, how often each was restarted and how often a frame kicked it out of its guest; `console` prints the end of one's console, escape sequences taken out; `attach`, from an SSH session with a terminal (`ssh -t`), is the console live and typed at, until ^] detaches; `send` types at it now, as at a terminal -- at a login prompt too, which a script's line waits for in vain -- and from then on for that boot `exec`'s lines go in as they are typed; `exec` types a line and prints the answer, up to the prompt printed after the line went in (10 s by default), and waits to type it for a shell's prompt until `send` has typed; `wait` returns once the current boot's console shows `text` -- with `boot=N`, not before the VM's Nth restart, what a script waits for after typing `reboot` -- or it stops; a `disk=` with `:ro` is read-only to the guest; `restart` boots it again from its files, running or stopped, and `restart` on `start` does that each time the guest resets itself (a reboot), at most five times a minute; `stop` stops it and prints how it ended. `log` also sends its console to the kernel log (`hvvm<id>| ...`). `hv off` will not turn the extension off under a running guest, and `rmmod hv` stops them all first (x86-64, AMD-V). See [Guests that stay up](hypervisor.md#guests-that-stay-up) |
| `hv bench [cpu=N] [exits=N] [pages=N] [flush] [profile]` | What a VM exit costs: a guest of nothing but CPUIDs, each after a read of `pages` pages, on a task bound to `cpu` (the extension's last CPU by default), timed -- `flush` flushes the whole TLB on every entry, as before ASIDs, and `profile` splits an entry into its parts (x86-64, AMD-V). See [What an exit costs](hypervisor.md#what-an-exit-costs) |
| `hv forward [add <port> <vm> <guest-port> \| del <port>]` | A port of nos's relayed to a port of a `net` guest's -- how a guest's server is reached from outside. `hv forward` alone lists them, with the connections each has carried (x86-64, AMD-V). The other way, a `net` guest reaches what nos reaches through NAT, and the switch answers its DHCP: `hv list` says which address the guests go out from and counts the DHCP answers. See [A network](hypervisor.md#a-network) |
| `hv help` | Every `hv` subcommand, a line each |
